// Reservation store with TypeScript (optimized version)
...
import type { ReservationStoreState, ReservationCartPart, Business, Block } from "../types";
...
export const actions = {
    ...
    getServicePrice(): string {
        const state = store.get();
        return getPrice(state.service?.priceOption, state.currency, getLocale());
    },
};
